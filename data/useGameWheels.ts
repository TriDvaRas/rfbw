...
import { GamePlayer, GameWheel } from '../database/db';
...
export default function useGameWheels(gameId?: string) {
  const { data, mutate, error: _error } = useSWR<GameWheel[], AxiosError<ApiError>>(gameId ? `/api/games/${gameId}/wheels` : null, fetcher);
