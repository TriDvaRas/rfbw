...
export default function useAudio(audioId?: string) {
  const { data, mutate, error: _error } = useSWR<Audio, AxiosError<ApiError>>(audioId ? `/api/audios/${audioId}/meta` : null, fetcher);
